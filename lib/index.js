...
exports["default"] = function () {
  return {
    ...
    reportTestDone: function reportTestDone(name, testRunInfo) {
      ...
      if (loggingLevel === _constLoggingLevels2["default"].TEST || _constLoggingLevels2["default"].DEBUG) {
        this.slack.addMessage(message);
        if (testRunInfo.screenshots) {
          // console.log("===>", testRunInfo.screenshots.length)
          var uploadAndSharePromise = new Promise(function (resolve, reject) {
            testRunInfo.screenshots.map(function (screenshotObj) {
              _this.slack.uploadAndShareFile(screenshotObj.screenshotPath).then(function (response) {
                if (!_config2["default"].quietMode) console.log("Completed share:", response.ok);
                resolve(true);
              })["catch"](function (err) {
                return console.log("Error uploading and sharing screenshots", err);
              });
            });
          });
          // console.log("===> pre-push", this.slack.callbackMessageReady)
          this.slack.callbackMessageReady.push(uploadAndSharePromise);
          // console.log("===> push", this.slack.callbackMessageReady)
        } else if (testRunInfo.screenshotPath) {
            var uploadAndSharePromise = new Promise(function (resolve, reject) {
              _this.slack.uploadAndShareFile(testRunInfo.screenshotPath).then(function (response) {
                if (!_config2["default"].quietMode) console.log("Completed share:", response.ok);
                resolve(true);
              })["catch"](function (err) {
                return console.log("Error uploading and sharing screenshot", err);
              });
            });
            this.slack.callbackMessageReady.push(uploadAndSharePromise);
          }
